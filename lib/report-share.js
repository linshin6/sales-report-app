...
async function shortenReportUrl(longUrl) {
  if (!longUrl) return '';
  // 1. da.gd (Instant 302 redirect, 0s delay)
  try {
  ...
  // 2. cleanuri (Instant 301 redirect, 0s delay)
  try {
    ...
    if (cleanRes.ok) {
      ...
      if (data && data.result_url && data.result_url.startsWith('http')) return data.result_url;
    }
  } catch (e) {}
