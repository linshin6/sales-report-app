...
async function generateCleanReportUrl(req, payload) {
  ...
  if (kvUrl && kvToken) {
    try {
      ...
      console.warn('KV save failed, falling back to secure URL compression:', eKv.message);
    }
  }

  // 2. Fallback an toàn: Mã hóa nén trực tiếp vào URL Hash (Zero-Server, hoàn toàn bảo mật)
